...
def main(cfg: DictConfig):
    ...
    try:
        ...
        if dataset_file is None:
            raise ValueError(
                "data.dataset_file must be set for dw HMC "
        ...
        if loaded.metadata is not None and loaded.metadata.theta_names:
            parsed_theta_names = [j.replace(",", "") for j in loaded.metadata.theta_names.split(" ")]
            if len(parsed_theta_names) == posterior_samples_unscaled.shape[-1]:
                theta_names = parsed_theta_names
        ...
        sbc_labels = (
            corner_labels if corner_labels is not None and len(corner_labels) == ranks.shape[1] else None
        )
