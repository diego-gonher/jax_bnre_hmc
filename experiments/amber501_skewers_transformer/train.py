...
@hydra.main(
    config_path="../../configs/amber501_skewers_transformer",
    config_name="train",
    version_base="1.3",
)
def main(cfg: DictConfig):
    ...
    if dataset_file is None:
        raise ValueError(
            "data.dataset_file must be set for amber501_skewers_transformer experiment "
    ...
    # Theta: same MinMaxScaler(-1, 1) as amber501_skewers MLP
    ...
    model = RatioEstimatorTransformer(
        ...
        head_hidden_dims=tuple(cfg.model.hidden_dims),
        head_activation=str(cfg.model.activation),
    ...
    if best_dir.exists() and best_meta_path.exists():
        ...
        if abs(recomputed_val_loss - expected_best_val_loss) < 1e-5:
            ...
            print(
                "  ⚠ Warning: Validation loss mismatch, likely due to random seed "
                "mismatch for shuffling!"
            )
