...
from jax_bnre_hmc.model import RatioEstimatorCNN1D
from jax_bnre_hmc.train import TrainConfig, train


@hydra.main(config_path="../../configs/sinusoid_cnn", config_name="train", version_base="1.3")
def main(cfg: DictConfig):
    ...
    if dataset_file is None:
        raise ValueError(
            "data.dataset_file must be set for sinusoid_cnn experiment "
    ...
    if mask_train_raw is None or mask_val_raw is None or mask_test_raw is None:
        raise ValueError(
            "Sinusoid CNN dataset must provide x_train_mask, x_val_mask, x_test_mask "
    ...
    # Model
    # -----------------------------
    model = RatioEstimatorCNN1D(
        conv_channels=tuple(cfg.model.conv_channels),
        kernel_sizes=tuple(cfg.model.kernel_sizes),
        dilations=tuple(cfg.model.dilations),
        conv_activation=str(cfg.model.conv_activation),
        conv_norm=str(cfg.model.conv_norm),
        conv_dropout_rate=float(cfg.model.conv_dropout_rate),
